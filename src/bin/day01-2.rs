...
fn main() {
    let total = io::stdin()
        ...
        .map(calc_required_fuel_recursive)
        .fold(0, |acc, x| acc + x);

    println!("{}", total);
}

fn calc_required_fuel_recursive(mass: i32) -> i32 {
    let fuel = mass / 3 - 2;
    match fuel {
        x if x <= 0 => 0,
        _ => fuel + calc_required_fuel_recursive(fuel),
    }
}

#[cfg(test)]
mod day01_2_tests {
    use super::calc_required_fuel_recursive;

    #[test]
    fn test_mass_14_requires_2_fuel() {
        assert_eq!(calc_required_fuel_recursive(14), 2);
    }

    #[test]
    fn test_mass_1969_requires_966_fuel() {
        assert_eq!(calc_required_fuel_recursive(1969), 966);
    }

    #[test]
    fn test_mass_100756_requires_50346_fuel() {
        assert_eq!(calc_required_fuel_recursive(100756), 50346);
    }
}
